...
## Current Sprint
- Completed Audio Streaming & Recording Pipeline Hardening (Cross-browser MediaRecorder MIME probing for Safari/iOS compatibility, resilient pause/resume with amber visual indicator, Multer audio file filter and 50MB error interceptor, speech recognition language persistence fix, Deepgram disconnect/heartbeat guards, and automatic unmount teardown).
...
## Completed Features
- **Audio Intelligence & Recording Pipeline**: Cross-browser codec probing (`getSupportedAudioMimeType` with WebM, MP4, AAC, OGG Opus fallback), hardened pause/resume with distinct amber UI states and timer preservation, Multer server-side file filter with structured 400 error handling, downstream language code normalization across Whisper & OpenAI prompts, unmount cleanup hook preventing orphaned background audio recordings, and granular `VoiceRecorder` error state banners.
