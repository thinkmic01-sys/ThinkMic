...
export default function AdminUserDetail() {
    ...
    return (
        <div className="w-full p-4 sm:p-6 md:p-8 pb-20">
            ...
            {/* Lecturer roster - only shown for a user whose role carries schemas.manage_own,
                since that's what makes "their students" a meaningful concept for this user. */}
            {profile.roleId?.permissions?.includes('schemas.manage_own') && (
                <LecturerStudentsPanel lecturerId={userId} showToast={showToast} />
            )}
...
// Which students this lecturer's schemas.manage_own forms are visible to - a student can be
// assigned to more than one lecturer (see User.assignedLecturers), so this is a roster
// membership toggle, not an exclusive ownership transfer.
function LecturerStudentsPanel({ lecturerId, showToast }) {
    ...
    useEffect(() => {
        api.get(`/admin/users/${lecturerId}/students`)
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                const res = await api.get('/admin/users', { params: { search: query.trim() } });
    ...
    const saveStudentIds = async (nextIds) => {
        const { data } = await api.patch(`/admin/users/${lecturerId}/students`, { studentIds: nextIds });
    ...
    return (
        <SectionCard title="Assigned Students" count={loading ? undefined : students.length}>
            ...
            {loading ? (
                ...
                <EmptyRow label="No students assigned yet." />
            ) : (
                students.map((s) => (
                    <div key={s._id} className="px-4 sm:px-5 py-3 flex items-center justify-between gap-2">
                        <div className="min-w-0">
