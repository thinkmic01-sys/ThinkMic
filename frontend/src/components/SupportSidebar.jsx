...
export default function SupportSidebar({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-y-0 right-0 w-full sm:w-[400px] bg-white shadow-[-5px_0_25px_rgba(0,0,0,0.1)] z-50 flex flex-col transform transition-transform duration-300">
            {/* Header */}
            <div className="bg-[#222777] shrink-0 text-white">
                <div className="h-16 flex items-center justify-between px-4 sm:px-6">
                    <div className="flex items-center gap-3 min-w-0">
                        <div className="relative shrink-0">
                            <span className="material-symbols-outlined text-[24px]">support_agent</span>
                            <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-[#00c2cb] border-2 border-[#222777] animate-pulse"></span>
                        </div>
                        <div className="min-w-0">
                            <h2 className="font-bold text-[15px] sm:text-[16px] truncate">Support Chat</h2>
                            <span className="text-[10px] sm:text-[11px] text-[#e6fbfc] opacity-90 truncate block">🟢 Online • Typically replies in minutes</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                        {ticket && (
                            <>
                                <span className={`text-[10px] font-bold px-2 py-1 rounded-full whitespace-nowrap ${ticket.status === 'open' ? 'bg-[#00c2cb]/20 text-white' : 'bg-white/15 text-white/80'}`}>
                    ...
                    {[{ id: 'chat', label: 'Live Chat', icon: 'forum' }, { id: 'help', label: 'Instant Help', icon: 'help_outline' }].map(tab => (
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[12px] font-bold transition-colors border-b-2 ${
                                activeTab === tab.id ? 'text-white border-[#00c2cb]' : 'text-white/50 border-transparent hover:text-white/80'
                            }`}
                        >
            ...
            {activeTab === 'chat' ? (
                            ...
                            <>
                                {messages.length === 0 && isTicketOpen ? (
                                    <div className="text-center my-auto flex flex-col items-center gap-4 w-full">
                                        <span className="material-symbols-outlined text-4xl text-[#222777]">forum</span>
                                        <div>
                                        ...
                                        <div className="flex flex-col gap-2 w-full">
                                            {QUICK_TOPICS.map(topic => (
                                                <button
                                                    ...
                                                    disabled={isSending}
                                                    className="w-full text-left bg-white border border-[#e0e2eb] rounded-lg px-3 py-2.5 text-[12px] font-semibold text-[#181c22] hover:border-[#00c2cb] hover:bg-[#e6fbfc]/30 transition-colors flex items-center gap-2 disabled:opacity-50"
                                                >
                                        ...
                                        return (
                                            <div key={msg._id || idx} className={`flex flex-col max-w-[85%] ${isMine ? 'self-end items-end' : 'self-start items-start'}`}>
                                                ...
                                                <span className="text-[10px] text-[#777682] font-semibold mt-1 px-1 flex items-center gap-1">
                                                    ...
                                                    {isMine && <span className="material-symbols-outlined text-[12px] text-[#00c2cb]">check_circle</span>}
                                                </span>
                                            </div>
                                        );
                                    })
                                ...
                                {ticket && ticket.status === 'closed' && (
                                    <div className="bg-white border border-[#e0e2eb] rounded-xl p-4 sm:p-5 shadow-sm flex flex-col items-center text-center gap-3">
                                        ...
                                        {ticket.rating ? (
                                            ...
                                            <>
                                                ...
                                                <textarea
                                                    ...
                                                    rows={2}
                                                    className="w-full bg-[#f9f9ff] border border-[#c7c5d3] rounded-lg px-3 py-2 text-[12px] focus:outline-none focus:border-[#222777] resize-none disabled:opacity-50"
                                                />
                                                <button
                                                    onClick={handleSubmitRating}
                                                    disabled={ratingValue === 0 || isSubmittingRating}
                                                    className="w-full py-2 bg-[#222777] text-white rounded-lg text-[12px] font-bold disabled:opacity-50 hover:bg-[#3a3f8f] transition-colors flex items-center justify-center gap-2"
                                                >
                                        ...
                                        <button
                                            ...
                                            className="w-full py-2 border border-[#c7c5d3] text-[#222777] rounded-lg text-[12px] font-bold hover:bg-[#f1f3fc] transition-colors flex items-center justify-center gap-2"
                                        >
                        ...
                        <form onSubmit={handleSendMessage} className="p-4 sm:p-5 bg-white border-t border-[#e0e2eb] shrink-0">
                            <div className="flex gap-2 items-end">
                                <textarea
                                    ...
                                    className="flex-1 bg-[#f9f9ff] border border-[#c7c5d3] rounded-2xl px-4 py-2.5 text-[13px] sm:text-[14px] focus:outline-none focus:border-[#222777] focus:ring-1 focus:ring-[#222777] transition-shadow resize-none max-h-28 overflow-y-auto"
                                />
                                <button
                                    type="submit"
                ...
                <div className="flex-1 overflow-y-auto flex flex-col bg-[#f9f9ff] min-h-0">
                    <div className="p-4 sm:p-5 border-b border-[#e0e2eb] bg-white shrink-0">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-3 py-2.5 rounded-lg border border-[#c7c5d3] bg-[#f9f9ff] text-[13px] focus:outline-none focus:border-[#222777] focus:ring-1 focus:ring-[#222777]"
                            />
                        </div>
                    </div>
                    ...
                    <div className="p-4 border-t border-[#e0e2eb] bg-white shrink-0">
                        <button onClick={() => setActiveTab('chat')} className="w-full text-[12px] font-bold text-[#222777] hover:underline flex items-center justify-center gap-1.5">
