...
- **Two Mongoose/MongoDB landmines specific to `Transcript`:** (1) `Transcript.text` has `required: true`, which Mongoose enforces as non-empty on `.create()` (an empty string fails validation) - `initLiveSession` seeds a placeholder string instead, which the first autosave PATCH (uses `findOneAndUpdate`, which does not run validators by default) immediately overwrites. (2) `Transcript.text` is a text-indexed field (`text: true`), so MongoDB treats a field literally named `language` on that document as its reserved per-document stemming-language override - passing a BCP-47 locale like `en-US` (as opposed to a Mongo-recognized language name) throws `language override unsupported`. Never set `language` when creating a `Transcript` for a live/bypass session.
