...
const initialResults = [
    {
        ...
        // Hidden attributes for our filters to use!
...
export default function ResearchResults() {
    ...
    const handleAddQuery = () => {
        ...
        if (newQueryText && newQueryText.trim() !== "") {
            const newId = Date.now(); // Generate a temporary unique ID
            setQueries(prev => [
                ...prev,
                { id: newId, text: newQueryText, status: "running", results: 0 }
            ]);
            setActiveQueryId(newId); // Automatically select the new query
        }
    };
    ...
    const currentResults = initialResults.filter(result => {
        ...
        // (Language filter would go here, but all our mock data is English)
        return true;
    });
    ...
    return (
        <div className="flex w-full h-[calc(100vh-64px)] bg-[#f9f9ff] p-4 md:p-6 gap-4 overflow-hidden">

            {/* COLUMN 1: Active Queries (25%) */}
            <section className="w-1/4 flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm shrink-0">
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                        <span className="material-symbols-outlined text-primary">list_alt</span> Active Queries
                    </h2>
                    <span className="bg-gray-100 text-gray-600 font-mono px-2 py-0.5 rounded text-[11px] font-bold">Session 4B</span>
                </div>
            ...
            {/* COLUMN 2: Results Grid (55%) */}
            <section className="flex-1 flex flex-col relative bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">

                {/* Search Config Bar */}
                <div className="p-4 border-b border-gray-200 bg-gray-50 flex flex-wrap items-center justify-between gap-4 z-10 shrink-0">
                    <div className="flex items-center gap-3">
                        <span className="text-sm font-bold text-gray-900 mr-2">Filters:</span>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                            {currentResults.map((result) => {
                                ...
                                return (
                                    <div
                                        key={result.id}
                                        onClick={() => setActiveSourceId(result.id)}
                                        className={`bg-white rounded-xl p-5 border relative group cursor-pointer transition-all flex flex-col h-full shadow-sm
                      ...
                      ${isSelected ? 'bg-primary/5' : ''}`}
                                    >
                                        <input
                                            ...
                                            className="absolute top-5 right-5 w-4 h-4 text-primary bg-white border-gray-300 rounded focus:ring-primary cursor-pointer z-10"
                                        />
                                        ...
                                        <div className="flex items-center gap-2 mb-3 pr-8">
                                            ...
                                            <span className="font-mono text-[11px] text-gray-500 truncate font-bold">{result.domain}</span>
                        ...
                        {result.type}
                                        ...
                                        <h3 className="text-base font-bold text-gray-900 group-hover:text-primary transition-colors mb-2 pr-6 leading-tight">
                                            {result.title}
                                        </h3>
                                        <p className="text-[13px] text-gray-600 line-clamp-3 leading-relaxed flex-1 mb-4">
                                            {result.snippet}
                                        </p>
                ...
                <div className="absolute bottom-0 w-full bg-white border-t border-gray-200 p-4 flex justify-between items-center shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-cyan/10 flex items-center justify-center shadow-[0_0_15px_rgba(0,194,203,0.3)]">
                            <span className="material-symbols-outlined text-cyan text-[20px]">auto_awesome</span>
                        </div>
                        <div>
                            <p className="text-sm text-gray-900"><strong className="font-bold text-primary">{selectedResultIds.length}</strong> of 12 results selected</p>
            ...
            {/* COLUMN 3: Source Detail (20%) */}
            <section className="w-1/5 flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm shrink-0 min-w-[280px]">
                ...
                <div className="flex-1 overflow-y-auto p-5">
                    {activeSourceDetail ? (
                        <div className="space-y-6">
                            <div>
                                <p className="font-mono text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-2">
                                    Selected Domain
                                </p>
                                <div className="flex items-center gap-3">
                        ...
                        <span className="material-symbols-outlined text-gray-500 text-[20px]">
                            ...
                            <div>
                                <p className="font-mono text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-3">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {activeSourceDetail.entities.map((entity, i) => (
                                        <span
                                            key={i}
                                            className="bg-gray-50 text-gray-700 font-mono text-[11px] font-bold px-2 py-1.5 rounded border border-gray-200"
                                        >
                            {entity}
                        </span>
                                    ))}
                            ...
                            {activeSourceDetail.metadata.author}
                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span className="text-gray-900 font-bold">
                            {activeSourceDetail.metadata.published}
                        </span>
                                    </div>
                        ...
                        <span className="text-gray-500 font-semibold">
                            ...
                            {activeSourceDetail.metadata.reliability}
                        </span>
                                    </div>
                                </div>
                            </div>
