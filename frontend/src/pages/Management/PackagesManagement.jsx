...
export default function PackagesManagement() {
    ...
    // Coin packages - same pattern as above, kept as separate state/handlers since the
    // fields differ (coins instead of storage/minutes/searches) even though they live on
    // this same admin page and share the packages.manage permission.
    const [coinPackages, setCoinPackages] = useState([]);
    const [isCoinLoading, setIsCoinLoading] = useState(true);
    const [coinForm, setCoinForm] = useState(emptyCoinForm);
    ...
    const fetchCoinPackages = async () => {
        ...
        try {
            const res = await api.get('/admin/coin-packages');
            setCoinPackages(res.data.coinPackages || []);
        } catch (err) {
            showToast('Failed to load coin packages.', 'error');
        } finally {
    ...
    // --- Coin package handlers (mirror the resource-package ones above) ---

    const handleCreateCoin = async () => {
        if (!coinForm.name.trim()) return showToast('Coin package name is required.', 'error');
        if ([coinForm.coins, coinForm.priceUSD].some((v) => v === '' || Number(v) < 0)) {
            return showToast('Coins and price must both be filled in with non-negative numbers.', 'error');
        }
        setIsCoinSaving(true);
        try {
            const res = await api.post('/admin/coin-packages', {
                name: coinForm.name.trim(),
                description: coinForm.description.trim(),
                coins: Number(coinForm.coins),
                priceUSD: Number(coinForm.priceUSD)
            });
            setCoinPackages((prev) => [...prev, res.data.coinPackage].sort((a, b) => a.priceUSD - b.priceUSD));
            setCoinForm(emptyCoinForm);
            showToast('Coin package created.', 'success');
        } catch (err) {
            showToast(err.response?.data?.message || 'Failed to create coin package.', 'error');
        } finally {
            setIsCoinSaving(false);
        }
    };

    const startEditCoin = (pkg) => {
        setCoinEditingId(pkg._id);
        setCoinEditForm({
            name: pkg.name,
            description: pkg.description || '',
            coins: pkg.coins,
    ...
    const cancelEditCoin = () => {
        setCoinEditingId(null);
        setCoinEditForm(emptyCoinForm);
    };

    const saveEditCoin = async (id) => {
        if (!coinEditForm.name.trim()) return showToast('Coin package name is required.', 'error');
        try {
            const res = await api.patch(`/admin/coin-packages/${id}`, {
                name: coinEditForm.name.trim(),
                description: coinEditForm.description.trim(),
                coins: Number(coinEditForm.coins),
                priceUSD: Number(coinEditForm.priceUSD)
            });
            setCoinPackages((prev) => prev.map((p) => p._id === id ? res.data.coinPackage : p).sort((a, b) => a.priceUSD - b.priceUSD));
            cancelEditCoin();
            showToast('Coin package updated.', 'success');
        } catch (err) {
            showToast(err.response?.data?.message || 'Failed to update coin package.', 'error');
        }
    };

    const toggleActiveCoin = async (pkg) => {
        try {
            const res = await api.patch(`/admin/coin-packages/${pkg._id}`, { isActive: !pkg.isActive });
            setCoinPackages((prev) => prev.map((p) => p._id === pkg._id ? res.data.coinPackage : p));
            showToast(res.data.coinPackage.isActive ? 'Coin package activated.' : 'Coin package deactivated.', 'success');
        } catch (err) {
            showToast(err.response?.data?.message || 'Failed to update coin package.', 'error');
        }
    };

    const handleDeleteCoin = async (pkg) => {
        try {
            await api.delete(`/admin/coin-packages/${pkg._id}`);
            setCoinPackages((prev) => prev.filter((p) => p._id !== pkg._id));
    ...
    return (
        <div className="w-full p-4 sm:p-6 md:p-8 flex flex-col pb-20 min-h-full">
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-xl border border-[#e0e2eb] p-8 text-center mb-10">
            ...
            <div className="mb-5 sm:mb-6 pt-2 border-t border-[#e0e2eb]">
                <h2 className="text-2xl sm:text-3xl font-bold text-[#075e51] mb-1 tracking-tight mt-6">Coin Packages</h2>
                <p className="text-[#777682] text-sm sm:text-base">
                    Define how many coins a user gets for a set USD price. Active coin packages appear when a user clicks their coin balance in the top navigation bar.
                </p>
            </div>

            <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(58,63,143,0.05)] border border-[#e0e2eb] p-5 sm:p-6 mb-6">
                <label className="block text-[12px] font-bold text-[#777682] uppercase tracking-wide mb-3">Add Coin Package</label>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                    <div>
                        <label className="block text-[10px] font-bold text-[#464651] uppercase tracking-wide mb-1.5">Name</label>
                        <input type="text" value={coinForm.name} onChange={(e) => setCoinForm((f) => ({ ...f, name: e.target.value }))} placeholder="e.g. Coin Bundle" className="w-full border border-[#c7c5d3] rounded-md py-2.5 px-3 text-[14px] outline-none focus:border-[#075e51] focus:ring-1 focus:ring-[#075e51]" />
                    </div>
                    <div className="lg:col-span-1">
                        <label className="block text-[10px] font-bold text-[#464651] uppercase tracking-wide mb-1.5">Description <span className="normal-case font-normal text-[#777682]">(optional)</span></label>
                        <input type="text" value={coinForm.description} onChange={(e) => setCoinForm((f) => ({ ...f, description: e.target.value }))} placeholder="Short one-line pitch" className="w-full border border-[#c7c5d3] rounded-md py-2.5 px-3 text-[14px] outline-none focus:border-[#075e51] focus:ring-1 focus:ring-[#075e51]" />
                    </div>
                    <div>
                        <label className="block text-[10px] font-bold text-[#464651] uppercase tracking-wide mb-1.5">Coins</label>
                        <input type="number" min="0" value={coinForm.coins} onChange={(e) => setCoinForm((f) => ({ ...f, coins: e.target.value }))} className="w-full border border-[#c7c5d3] rounded-md py-2.5 px-3 text-[14px] outline-none focus:border-[#075e51] focus:ring-1 focus:ring-[#075e51]" />
                    </div>
                    <div>
                        <label className="block text-[10px] font-bold text-[#464651] uppercase tracking-wide mb-1.5">Price (USD)</label>
                        <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#777682] text-[14px]">$</span>
                            <input type="number" min="0" step="0.01" value={coinForm.priceUSD} onChange={(e) => setCoinForm((f) => ({ ...f, priceUSD: e.target.value }))} className="w-full border border-[#c7c5d3] rounded-md py-2.5 pl-7 pr-3 text-[14px] outline-none focus:border-[#075e51] focus:ring-1 focus:ring-[#075e51]" />
                        </div>
                    </div>
                </div>
                <button
                    onClick={handleCreateCoin}
                    disabled={isCoinSaving}
                    className="bg-[#075e51] text-white text-[13px] font-bold px-5 py-2.5 rounded-md hover:bg-[#097969] transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
                >
                    <span className="material-symbols-outlined text-[18px]">add</span>
                    {isCoinSaving ? 'Adding...' : 'Add Coin Package'}
                </button>
            </div>

            {isCoinLoading ? (
                <p className="text-center text-[#777682] py-8 text-sm">Loading coin packages...</p>
            ) : coinPackages.length === 0 ? (
                <div className="bg-white rounded-xl border border-[#e0e2eb] p-8 text-center">
                    <p className="text-[#777682] text-sm">No coin packages yet. Add one above.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {coinPackages.map((pkg) => (
                        <div key={pkg._id} className={`bg-white rounded-xl shadow-[0_1px_4px_rgba(58,63,143,0.08)] border p-5 flex flex-col ${pkg.isActive ? 'border-[#e0e2eb]' : 'border-[#e0e2eb] opacity-60'}`}>
                            {coinEditingId === pkg._id ? (
                                <div className="flex flex-col gap-2.5">
                                    <input type="text" value={coinEditForm.name} onChange={(e) => setCoinEditForm((f) => ({ ...f, name: e.target.value }))} placeholder="Name" className="w-full border border-[#c7c5d3] rounded-md py-2 px-2.5 text-[13px] font-bold outline-none focus:border-[#075e51]" />
                                    <input type="text" value={coinEditForm.description} onChange={(e) => setCoinEditForm((f) => ({ ...f, description: e.target.value }))} placeholder="Description" className="w-full border border-[#c7c5d3] rounded-md py-2 px-2.5 text-[12px] outline-none focus:border-[#075e51]" />
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="block text-[9px] font-bold text-[#777682] uppercase mb-1">Coins</label>
                                            <input type="number" min="0" value={coinEditForm.coins} onChange={(e) => setCoinEditForm((f) => ({ ...f, coins: e.target.value }))} className="w-full border border-[#c7c5d3] rounded-md py-1.5 px-2 text-[12px] outline-none focus:border-[#075e51]" />
                                        </div>
                                        <div>
                                            <label className="block text-[9px] font-bold text-[#777682] uppercase mb-1">Price USD</label>
                                            <input type="number" min="0" step="0.01" value={coinEditForm.priceUSD} onChange={(e) => setCoinEditForm((f) => ({ ...f, priceUSD: e.target.value }))} className="w-full border border-[#c7c5d3] rounded-md py-1.5 px-2 text-[12px] outline-none focus:border-[#075e51]" />
                                        </div>
                                    </div>
                                    <div className="flex gap-2 mt-1">
                                        <button onClick={() => saveEditCoin(pkg._id)} className="flex-1 bg-[#075e51] text-white text-[12px] font-bold py-2 rounded-md hover:bg-[#097969] transition-colors">Save</button>
                                        <button onClick={cancelEditCoin} className="flex-1 border border-[#c7c5d3] text-[#464651] text-[12px] font-bold py-2 rounded-md hover:bg-[#f1f3fc] transition-colors">Cancel</button>
                                    </div>
                                </div>
                            ) : (
                                <>
                                    ...
                                    {pkg.description && <p className="text-[12px] text-[#777682] mb-3">{pkg.description}</p>}
                                    <div className="text-[28px] font-bold text-[#075e51] mb-3">${pkg.priceUSD}</div>
                                    <div className="flex flex-col gap-1.5 text-[13px] text-[#464651] mb-4">
                                        <span className="flex items-center gap-2"><span className="material-symbols-outlined text-[16px] text-[#EAB308]">toll</span> {pkg.coins.toLocaleString()} coins</span>
                                    </div>
                                    <div className="mt-auto flex gap-2 pt-3 border-t border-[#f1f3fc]">
                                        <button onClick={() => startEditCoin(pkg)} className="flex-1 text-[12px] font-bold text-[#075e51] border border-[#e0e2eb] hover:bg-[#f1f3fc] transition-colors py-2 rounded-md flex items-center justify-center gap-1">
                                            <span className="material-symbols-outlined text-[14px]">edit</span> Edit
                                        </button>
                                        <button onClick={() => toggleActiveCoin(pkg)} className="flex-1 text-[12px] font-bold text-[#854d0e] border border-[#EAB308]/50 bg-[#FEF9C3] hover:bg-[#d0f6f8] transition-colors py-2 rounded-md">
                                            {pkg.isActive ? 'Deactivate' : 'Activate'}
                                        </button>
                                        <button onClick={() => handleDeleteCoin(pkg)} className="text-[#ba1a1a] border border-[#ffb4ab] hover:bg-[#ffdad6] transition-colors py-2 px-2.5 rounded-md">
                                            <span className="material-symbols-outlined text-[16px]">delete</span>
                                        </button>
                                    </div>
                                </>
